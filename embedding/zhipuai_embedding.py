...
class ZhipuAIEmbeddings(BaseModel, Embeddings):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        try:
            values["client"] = zhipuai.ZhipuAI(api_key=values["zhipuai_api_key"])
    ...
    def _embed(self, texts: str) -> List[float]:
        ...
        try:
            print(texts)
            client = zhipuai.ZhipuAI(api_key=self.zhipuai_api_key)
        ...
        if hasattr(resp, "error") and hasattr(resp.error, "code") and resp.error.code != 200:
            raise ValueError(
